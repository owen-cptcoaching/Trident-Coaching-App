...
export const NutritionView: React.FC<NutritionViewProps> = ({ plan, isQuickView = false, onEnterFullView }) => {
  return (
    <div className="space-y-20">
      <div className="max-w-3xl flex flex-col md:flex-row md:items-start justify-between gap-6">
        <div>
          <div className="flex items-center gap-4 sm:gap-6 mb-4">
            <span className="text-5xl sm:text-8xl font-display italic text-stone-200 leading-none">02</span>
            <h2 className="text-3xl sm:text-6xl font-display font-black uppercase tracking-tighter leading-none italic">
              Nutrition Plan
            </h2>
          </div>
          <p className="text-lg sm:text-xl text-stone-500 font-serif italic mb-8 pl-0 sm:pl-24">
            {plan.title}
          </p>
        </div>
      ...
      <div className="pl-0 sm:pl-24 max-w-3xl grid grid-cols-2 md:grid-cols-4 gap-4 sm:gap-8">
            <div className="border-t-4 border-stone-900 pt-6">
                ...
                <p className="text-2xl sm:text-4xl font-mono font-black">{plan.dailyCalories}<span className="text-xs sm:text-sm font-sans ml-1 text-stone-300 italic uppercase">kcal</span></p>
            </div>
            {[
                { label: 'Protein', val: plan.macros.protein, color: 'border-stone-900' },
                ...
                <div key={macro.label} className={cn("border-t-4 pt-6 transition-all", macro.color)}>
                    ...
                    <p className="text-2xl sm:text-4xl font-mono font-black">{macro.val}<span className="text-xs sm:text-sm font-sans ml-1 text-stone-300 italic uppercase">g</span></p>
                </div>
            ))}
        </div>

      {isQuickView && (
        <div className="pl-0 sm:pl-24 mt-8 flex flex-wrap gap-2">
